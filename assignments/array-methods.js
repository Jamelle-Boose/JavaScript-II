...
// Lets start with an empty array
let fullName = []

// forEach method and combining names
...
// forEach method 
runners.forEach(element => {
  return allCaps.push(`${element.first_name.toUpperCase()}`)
})
...
let largeShirts = runners.filter(size => size.shirt_size === "L")
...
let ticketPriceTotal = runners.reduce((acc, user) => acc + user.donation, 0)
...
// Problem 1 - Gather email addresses for a mailing list
let mailingAddress = []

runners.forEach(element => {
  return mailingAddress.push(`${element.email}`)
})
...
// Problem 2 - Create a VIP donors list

let vipDonors = runners.filter(vip => vip.donation >= 200)
...
// Problem 3 - Donation sizes

// How many ppl gave between 0-100
let bronzeDonationCount = runners.filter(bronze => bronze.donation <= 100)/*?*/

let bronzeClub = bronzeDonationCount.length

console.log(`We have ${bronzeClub} members in the Bronze club.`)



// How many ppl gave between 101-200
let silverDonationCount = runners.filter(silver => silver.donation >= 101 && silver.donation <= 200) /*?*/
...
// How many ppl gave between 201-300
let goldDonationCount = runners.filter(gold => gold.donation >= 201)
